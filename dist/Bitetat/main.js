...
var HeaderComponent = /** @class */ (function () {
    ...
    HeaderComponent.prototype.ngOnInit = function () {
        ...
        this.utils.profileStatus.subscribe(function (data) {
            debugger;
            _this.adminData = _this.utils.get('bitetat_admin_data');
    ...
    HeaderComponent.prototype.logout = function () {
        var _this = this;
        debugger;
        var admin = this.utils.get('bitetat_admin_data');
        this.api.logout({
            id: admin._id
        }).subscribe(function (data) {
            _this.utils.alert('success', data['message']);
            _this.router.navigate(['/login']);
            _this.utils.removeData('bitetat_admin_data');
        }, function (error) {
            console.log(error);
            if (error['status'] == 400) {
                _this.router.navigate(['/login']);
...
var EditProfileComponent = /** @class */ (function () {
    function EditProfileComponent(utils, fb, api, router) {
        ...
        this.router = router;
        this.submitted = false;
        this.adminData = this.utils.get('bitetat_admin_data');
    ...
    //     data=> {
    //       this.utils.removeData('bitetat_admin_data');
    //       this.utils.set('bitetat_admin_data', data['data']);
    ...
    EditProfileComponent.prototype.submit = function () {
        ...
        this.api.updateProfile(uploadData).subscribe(function (data) {
            _this.utils.removeData('bitetat_admin_data');
            _this.utils.set('bitetat_admin_data', data['response']);
            _this.utils.alert('success', data['message']);
            _this.router.navigate(['/profile']);
        }, function (error) {
            console.log(error);
...
var UserManagementComponent = /** @class */ (function () {
    function UserManagementComponent(api, fb, utils, router, excelService) {
        ...
        this.newEmployee = fb.group({
            ...
            validator: this.MustRePassMatch('password', 'confirm_password')
        });
    }
    ...
    UserManagementComponent.prototype.addNewEmployee = function () {
        this.submitted = false;
        this.updateUser = false;
        $(".result").attr('src', 'assets/img/3002121059.jpg');
        this.newEmployee.get('password').setValidators([_angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required]);
        this.newEmployee.get('password').updateValueAndValidity();
        this.newEmployee.get('confirm_password').setValidators([_angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required]);
        this.newEmployee.get('confirm_password').updateValueAndValidity();
    ...
    Object.defineProperty(UserManagementComponent.prototype, "f", {
        get: function () { return this.newEmployee.controls; },
        enumerable: true,
        configurable: true
    });
    UserManagementComponent.prototype.createUser = function () {
        ...
        this.updateUser = false;
        this.submitted = true;
        var profileImage = $(".result").attr('src');
        ...
        var dataToSend = {
            ...
            contact_number: this.newEmployee.value.contact,
            profile_image: profileImage.includes('data:image/') ? profileImage : undefined,
        };
        this.api.createNewUser(dataToSend).subscribe(function (data) {
            _this.utils.alert('success', data['message']);
    ...
    UserManagementComponent.prototype.selectEditUser = function (user) {
        this.updateUser = true;
        this.selectedEditUser = user;
        this.submitted = false;
        this.newEmployee.patchValue({
            first_name: this.selectedEditUser.first_Name,
            last_name: this.selectedEditUser.last_Name,
            email: this.selectedEditUser.email,
            contact: this.selectedEditUser.contact_number,
        });
        $('.result').attr('src', src_environments_environment__WEBPACK_IMPORTED_MODULE_8__["environment"].baseUrl + "" + this.selectedEditUser.profile_image);
    };
    UserManagementComponent.prototype.updateSelectedUser = function () {
        var _this = this;
        this.newEmployee.get('password').setValidators([]);
        this.newEmployee.get('password').updateValueAndValidity();
        this.newEmployee.get('confirm_password').setValidators([]);
        this.newEmployee.get('confirm_password').updateValueAndValidity();
        var profileImage = $(".result").attr('src');
        this.submitted = true;
        if (this.newEmployee.invalid)
            return;
        var dataToSend = {
            id: this.selectedEditUser._id,
            first_Name: this.newEmployee.value.first_name,
            last_Name: this.newEmployee.value.last_name,
            email: this.newEmployee.value.email,
            password: this.newEmployee.value.password ? this.newEmployee.value.password : undefined,
            contact_number: this.newEmployee.value.contact,
            profile_image: profileImage.includes('data:image/') ? profileImage : undefined,
        };
        this.api.updateUser(dataToSend).subscribe(function (data) {
            _this.utils.alert('success', data['message']);
            // this.Userlist.push(data['data']);
            // this.dataSource = this.Userlist;
            // setTimeout(()=> {
    ...
    UserManagementComponent.prototype.MustRePassMatch = function (controlRePassName, matchingRePassControlName) {
        return function (formGroup) {
            var control = formGroup.controls[controlRePassName];
            var matchingRePassControl = formGroup.controls[matchingRePassControlName];
            if (matchingRePassControl.errors && !matchingRePassControl.errors.mustMatch) {
...
var LoginComponent = /** @class */ (function () {
    function LoginComponent(fb, utils, api, router) {
        ...
        this.router = router;
        this.submitted = false;
        var adminCredentials = this.utils.get('bitetat_admin_data');
    ...
    LoginComponent.prototype.login = function () {
        ...
        if (remember_me) {
            this.utils.set('bitetat_admin_data', this.loginForm.value);
        }
        else {
            this.utils.removeData('bitetat_admin_data');
        }
...
var AuthInterceptor = /** @class */ (function () {
    ...
    AuthInterceptor.prototype.intercept = function (req, next) {
        var adminData = localStorage.getItem('bitetat_admin_data');
        var token;
        if (adminData) {
            token = JSON.parse(localStorage.getItem('bitetat_admin_data')).access_token;
        }
        else {
            token = '';
        }
...
module.exports = __webpack_require__(/*! /home/fluper/Documents/1.office project/BitetatApp/bitetatApp_admin/src/main.ts */"./src/main.ts");
