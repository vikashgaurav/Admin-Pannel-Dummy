...
@Component({
  selector: 'app-agro-tech-spec',
  templateUrl: './agro-tech-spec.component.html',
  styleUrls: ['./agro-tech-spec.component.css']
})


export class AgroTechSpecComponent implements OnInit {
  ...
  getList() {
    this.api.get_list_of_education({ type: 4 }).subscribe(
      data => {
  ...
  submit_value() {
    ...
    this.api.add_list_data({ type: 4, name: this.name }).subscribe(
      data => {
        this.update_data = data['response'];
        this.utils.alert('success', data['message']);
        this.getList();
