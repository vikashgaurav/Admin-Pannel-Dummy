...
@Component({
  selector: 'app-eduction-list',
  templateUrl: './eduction-list.component.html',
  styleUrls: ['./eduction-list.component.css']
})



export class EductionListComponent implements OnInit {
  ...
  getList() {
    this.api.get_list_of_education({type:1}).subscribe(
      data => {
  ...
  submit_value(){
    ...
    this.api.add_list_data({ type: 1 ,name :this.name}).subscribe(
      data => {
        this.update_data = data['response'];
        this.utils.alert('success', data['message']);
        this.getList();
